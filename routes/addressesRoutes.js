...
const {invalidAddress, invalidUpdate} = require("../validation/addressValidation");
...
router.get("/", (req, res) => {
    ...
    .catch((error) => {
        res.status(500).json({"Error": "Could not get addresses", "Error message": error});
    });
});
...
router.post("/", (req, res) => {
    ...
    .catch((error) => {
        res.status(500).json({"Error": "Could not create addresses", "Error message": error});
    });
});
...
router.patch("/:id", (req, res) => {
    ...
    if(ObjectId.isValid(id)){
        ...
        if(updates.email)
            updateQuery["email"] = updates.email;
        
        if(updates.user.name)
            updateQuery["user.name"] = updates.user.name;
        ...
        .catch(error => {
            res.status(500).json({error: "Could not update the address", "Error message": error});
        });
    }else{
...
router.delete("/:id", (req, res) => {
    ...
    if(ObjectId.isValid(id)){
        ...
        .catch((error) => {
            res.status(500).json({"Error": "Could not delete address", "Error message": error});
        });
